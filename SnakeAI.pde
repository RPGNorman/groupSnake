...
GreedyView greedySnake;

Hamiltonian hamiltonianSnake;
...
void setup() {
  ...
  if(humanPlaying) {
    ...
    greedySnake = new GreedyView();
  }
  else if(isHamiltonian){
    hamiltonianSnake = new Hamiltonian();
  }
  else {
    pop = new Population(50000); //adjust size of population
  }
}
...
void draw() {
  ...
  if(humanPlaying) {
    ...
    if(snake.dead && humanPlaying) {
      ...
      //hamiltonianSnake.start();
      hamiltonianSnake.update();
      fill(150);
      textSize(20);
      graphButton.show();
    }
